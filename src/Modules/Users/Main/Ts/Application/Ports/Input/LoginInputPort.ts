...
export default class LoginInputPort implements ILoginUseCase
{
    public constructor
        (
            @Inject(IHashService)
    ...
    { }
    public async handleQuery(command: LoginCommand): Promise<Result<LoginDto>>
    {
        ...
        const isSamePassword = this._hashService.compare(user.password as never, passwordResult.value.value);
        ...
        const data = await this._tokenService.validate(accessToken);
        ...
        return Result.ok
            (
                new LoginDto
                    (
                        user.email?.value ?? user.email as never,
